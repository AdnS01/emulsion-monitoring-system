...
if(isset($_POST['Tf']) && isset($_POST['date1']) && isset($_POST['date2'])){
    ...
    $dateA = $_SESSION["date1"] = $_POST['date1'];
    $dateB = $_SESSION["date2"] = $_POST['date2'];
    $CnTf = $_SESSION["CnTf"];
...
if($_SESSION["DEFAULT"]){
    $CnTf = $_SESSION["CnTf"];
    $result = mysqli_query($conn, "SELECT * FROM $CnTf ORDER BY `ID` DESC");
    $row = mysqli_fetch_array($result);
    do{$tab[] = $row;}while($row = mysqli_fetch_assoc($result));
    //exporting
    $RESexp = mysqli_query($conn, "SELECT * FROM $CnTf ORDER BY `ID` ASC");
    while($ROWexp = mysqli_fetch_assoc($RESexp)){$Texp[] = $ROWexp;}
} else {
    $dateA = $_SESSION["date1"];
    $dateB = $_SESSION["date2"];
    $CnTf = $_SESSION["CnTf"];
    $result = mysqli_query($conn, "SELECT * FROM $CnTf WHERE `DATE` between '$dateA' AND '$dateB' ORDER BY `ID` DESC");
    $row = mysqli_fetch_array($result);
    ...
    if(!is_array($row)){
        ...
        do{$tab[] = $row;}while($row = mysqli_fetch_assoc($result));
        //exporting
        $RESexp = mysqli_query($conn, "SELECT * FROM $CnTf WHERE `DATE` between '$dateA' AND '$dateB' ORDER BY `ID` ASC");
        while($ROWexp = mysqli_fetch_assoc($RESexp)){$Texp[] = $ROWexp;}
        //var
        $erreur1 = false;
        $erreur2 = false;
...
<header>
    ...
    <div class="heroe">
        <h1 style = "text-shadow: 2px 4px 3px rgba(0,0,0,0.3);">Collection Concentration History</h1>
        <h2>
